...
func (a *Agent) FillContext(percent int) (int, error) {
	...
	if a.store != nil {
		if err := a.store.Save(a.id, next); err != nil {
...
func (a *Agent) ShowContext(out io.Writer) {
	...
	if meter.ContextLimit <= 0 {
		...
		fmt.Fprintf(out, "Состав: system ≈%d · диалог ≈%d (%d сообщ.) · наполнитель ≈%d (%d блок.)\n",
			systemTokens, dialogTokens, dialogMessages, fillerTokens, fillerMessages)
		return
	}
	...
	fmt.Fprintf(out, "  Состав: system ≈%d · диалог ≈%d (%d сообщ.) · наполнитель ≈%d (%d блок.) · %s\n",
		systemTokens, dialogTokens, dialogMessages, fillerTokens, fillerMessages, space)
...
func contextCommand(agent *Agent, line string, out io.Writer) bool {
	...
	case "/overflow":
		...
		agent.mu.Unlock()
		if fields[1] == "on" {
			fmt.Fprintln(out, "Переполнение: отправляем весь контекст в API, без сокращения. Возможен оплачиваемый запрос или отказ API.")
		} else {
