...
func runTerminal(ctx context.Context, agent *Agent, in io.Reader, out io.Writer, prompt string) error {
	...
	if prompt != "" {
		if contextCommand(agent, strings.TrimSpace(prompt), info) {
			return nil
		}
	...
	fmt.Fprintln(info, "Контекст модели: [+20%] [+50%] [+100%] · /fill N · /context.")
	...
	go func() {
		if edited {
			...
			readLine:
				line, err := readEditedLinePolling(reader, out, true, history...)
				if err != nil {
					if err == io.EOF {
						err = nil
					}
	...
	for {
		...
		if contextCommand(agent, line, info) {
			continue
		}
		switch line {
		case "":
