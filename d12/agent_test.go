...
func TestMemoryLayersAreSeparateAndInfluenceContext(t *testing.T) {
	...
	for _, entry := range []struct{ layer, key, value string }{
		...
		{"long", "knowledge.topic", "алгоритмы"},
	} {
	...
	for _, want := range []string{"черновик", "пятница", "алгоритмы", "Каков план?"} {
	...
	if snapshot.Working["deadline"] != "пятница" || snapshot.LongTerm["knowledge.topic"] != "алгоритмы" {
...
func TestAutomaticExtractorClassifiesAndPersistsFacts(t *testing.T) {
	...
	extractor := &scriptedExtractor{updates: []MemoryUpdate{{
		...
		Long:    LayerUpdate{Set: map[string]string{"decision.language": "Go"}},
	}, {}}}
	client := &captureClient{}
	...
	if layers.ShortTerm["current.file"] != "agent.go" || layers.Working["task.goal"] != "добавить память" || layers.LongTerm["decision.language"] != "Go" {
		t.Fatalf("layers=%+v", layers)
	}
	...
	for _, want := range []string{"agent.go", "добавить память", "Go"} {
