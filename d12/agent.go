...
func NewAgent(client ChatClient, system string, store LayerStore, id string, profiles ...UserProfile) *Agent {
	a := &Agent{client: client, system: strings.TrimSpace(system), store: store, id: id}
	if len(profiles) > 0 {
		a.profile = profiles[0]
	}
...
func (a *Agent) Profile() UserProfile {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.profile
}
...
func buildContext(system string, profile UserProfile, layers MemoryLayers, dialog []Message, prompt string) []Message {
	messages := make([]Message, 0, len(dialog)+5)
	if system != "" {
		messages = append(messages, Message{Role: "system", Content: system})
	}
	if profile.ID != "" {
		messages = append(messages, profileMessage(profile))
	}
	if len(layers.LongTerm) > 0 {
		messages = append(messages, memoryBlock("Долговременная память (факты, решения, знания)", layers.LongTerm))
	}
...
func (a *Agent) Ask(ctx context.Context, prompt string, emit func(string) error) (string, error) {
	...
	a.toolNotice = ""
	messages := buildContext(a.system, a.profile, a.layers, a.dialog, prompt)
