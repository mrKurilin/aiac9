...
- long: подтверждённые факты, решения и знания, полезные для будущих задач.
Профиль пользователя хранится отдельно и изменяется только явно. Не сохраняй в long просьбы о стиле, длине, языке, юморе или формате ответа.
