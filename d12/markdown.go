...
func terminalOutput(out io.Writer) bool {
	if terminal, ok := out.(interface{ isTerminal() bool }); ok {
...
func newMarkdownWriter(out io.Writer) *markdownWriter {
	...
	return &markdownWriter{out: out, styled: terminalOutput(out) && !noColor}
}
...
func (m *markdownWriter) line(line, end string) error {
	...
	if m.fence != "" {
		...
		line = inlineMarkdown.ReplaceAllStringFunc(line, func(source string) string {
			parts := inlineMarkdown.FindStringSubmatch(source)
			switch {
			case parts[1] != "":
				return "\x1b[36m" + parts[1] + "\x1b[0m"
			case parts[2] != "":
				return "\x1b[1m" + parts[2] + "\x1b[0m"
			case parts[3] != "":
				return "\x1b[1m" + parts[3] + "\x1b[0m"
			case parts[4] != "":
				return parts[4] + " (" + parts[5] + ")"
			default:
				return "\x1b[3m" + parts[6] + "\x1b[0m"
			}
		})
		if heading != nil {
