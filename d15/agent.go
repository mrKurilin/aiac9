...
var validationSuccessPattern = regexp.MustCompile(`(?i)(все\s+(тесты|проверки)\s+(успешн\p{L}*|прошл\p{L}*)|проверк\p{L}*\s+(успешн\p{L}*|пройден\p{L}*)|валидац\p{L}*\s+(успешн\p{L}*|пройден\p{L}*)|замечаний\s+нет|ошибок\s+нет|all\s+(tests|checks)\s+pass\p{L}*)`)
...
func confirmsSuccessfulValidation(prompt string) bool {
	return validationSuccessPattern.MatchString(prompt)
}
...
func cloneState(state *TaskState) *TaskState {
	...
	copy.Transitions = append([]TransitionRecord(nil), state.Transitions...)
...
func buildMessages(system string, invariants []Invariant, state TaskState, prompt string) []Message {
	...
	}{state.Goal, state.Stage, state.CurrentStep, state.ExpectedAction, state.CompletedSteps, state.PlanApproved, state.ImplementationComplete, state.ValidationPassed}
	...
	messages = append(messages, Message{Role: "system", Content: "TASK_STATE: " + string(encoded) +
		...
		"после завершения реализации — validation; после явно успешной проверки — done; при замечаниях проверки — execution. " +
...
const automaticExecutionPrompt = "Приступай к реализации по утверждённому плану прямо сейчас. Выполни все шаги этапа execution последовательно без промежуточных подтверждений пользователя и не останавливайся после каждого шага. Запрашивай пользователя только если без отсутствующих данных продолжение объективно невозможно. Если реализация полностью закончена, обязательно верни next_stage=\"validation\", current_step и expected_action для проверки. Оставь next_stage пустым только когда реализация действительно не завершена, и явно назови блокирующую причину."
...
func (a *Agent) applyDecision(next *TaskState, prompt string, decision modelDecision) TurnResult {
	...
	if next.Stage == StageExecution && target == StageValidation {
		_ = next.CompleteImplementation()
	}
	if next.Stage == StageValidation && target == StageDone && confirmsSuccessfulValidation(prompt) {
		_ = next.PassValidation()
	}
...
func (a *Agent) AskStream(ctx context.Context, prompt string, emit func(string) error) (TurnResult, error) {
	...
	if a.state.Stage == StagePlanning && implementationRequestPattern.MatchString(prompt) && !approvesPlan(prompt) {
		next := *cloneState(a.state)
		next.RejectTransition(StageExecution, "переход planning → execution запрещён: сначала согласуйте и явно утвердите план")
		result := TurnResult{
	...
	} else if a.state.Stage == StageValidation && confirmsSuccessfulValidation(prompt) {
		decision.NextStage = string(StageDone)
		decision.CurrentStep = ""
		decision.ExpectedAction = ""
	}
	next := *cloneState(a.state)
