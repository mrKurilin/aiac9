...
уточняющий вопрос.
Этапы переключаются автоматически, но только после контрольных условий.
Разрешённые переходы:
  planning → execution
  execution → validation | planning
  validation → done | execution
Условия продвижения: утверждённый план → завершённая реализация → успешная валидация.`
...
func printState(out io.Writer, state *TaskState) {
	...
	}{state.Goal, state.Stage, state.CurrentStep, state.ExpectedAction, state.CompletedSteps, state.Paused, state.PauseReason, state.PlanApproved, state.ImplementationComplete, state.ValidationPassed, state.Transitions}
...
func runTerminal(ctx context.Context, a *Agent, in io.Reader, out io.Writer) error {
	...
	fmt.Fprintln(display, "mrkai · controlled task lifecycle · /info — команды")
