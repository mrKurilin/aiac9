...
func (a *Agent) AskStream(ctx context.Context, prompt string, emit func(string) error) (string, error) {
	...
	// Compression runs before the next user request. If summarization fails,
	// the full history remains available and the normal answer still proceeds.
	if err := a.compactIfNeeded(ctx); err != nil {
		a.compressionWarning = err.Error()
	}
...
func (a *Agent) Reset() {
	...
	a.history = nil
	a.summary = ""
	a.compressionStats = CompressionStats{}
...
// Summary returns the separately stored compressed part of the conversation.
...
func (a *Agent) persist() {
	...
	if err := a.store.Save(a.id, ConversationState{Summary: a.summary, History: a.history}); err != nil {
