...
func (p *Pricing) At(at time.Time) (Prices, error) {
	...
	if p.Input == nil || p.Cached == nil || p.Output == nil {
		...
		if err != nil || endpoint.Scheme != "https" || endpoint.Host != "api.deepseek.com" {
			return rates, fmt.Errorf("для другого API задайте все три тарифа: DEEPSEEK_INPUT_PRICE, DEEPSEEK_CACHED_PRICE, DEEPSEEK_OUTPUT_PRICE")
		}
		switch p.Model {
		...
		default:
			return rates, fmt.Errorf("неизвестный тариф модели %q: задайте DEEPSEEK_INPUT_PRICE, DEEPSEEK_CACHED_PRICE и DEEPSEEK_OUTPUT_PRICE", p.Model)
		}
		utc := at.UTC()
		hour := utc.Hour()
		day := utc.Weekday()
