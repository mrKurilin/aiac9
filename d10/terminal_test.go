...
func TestTerminalSingleAndEOF(t *testing.T) {
	...
	if !strings.Contains(out.String(), "Ответ\n") {
		t.Fatal(out.String())
	}
...
func TestClearConversationView(t *testing.T) {
	var out bytes.Buffer
	if err := clearConversationView(&out, false); err != nil {
		t.Fatal(err)
	}
	if out.Len() != 0 {
		t.Fatalf("escape codes leaked into non-interactive output: %q", out.String())
	}
	if err := clearConversationView(&out, true); err != nil {
		t.Fatal(err)
	}
	if out.String() != "\x1b[2J\x1b[3J\x1b[H" {
		t.Fatalf("unexpected clear sequence: %q", out.String())
	}
}

func TestConversationBlocksHaveTwoEmptyLines(t *testing.T) {
	a := NewAgent(&terminalFake{}, "", nil, "spacing")
	var out bytes.Buffer
	if err := runTerminal(context.Background(), a, strings.NewReader("one\ntwo\n/exit\n"), &out, ""); err != nil {
		t.Fatal(err)
	}
	text := out.String()
	if !strings.Contains(text, "one\n\n\n─ ОТВЕТ") {
		t.Fatalf("message and answer are not separated by two empty lines:\n%s", text)
	}
	if !strings.Contains(text, "Ответ\n\n\n─ СООБЩЕНИЕ") {
		t.Fatalf("answer and next message are not separated by two empty lines:\n%s", text)
	}
}
