...
func (a *Agent) FillContext(percent int) (int, error) {
	...
	if a.keepLast == 0 {
		return 0, fmt.Errorf("при N=0 наполнитель не сохраняется; задайте N через /contextManagementStrategy")
	}
	...
	state := a.snapshot()
	state.History = a.trim(next)
	if err := a.commit(state); err != nil {
		return 0, err
	}
...
func (a *Agent) ShowContext(out io.Writer) {
	...
	if meter.ContextLimit <= 0 {
		...
		fmt.Fprintf(out, "Состав: system ≈%d · facts ≈%d · диалог ≈%d (%d сообщ.) · наполнитель ≈%d (%d блок.)\n",
			systemTokens, factsTokens, dialogTokens, dialogMessages, fillerTokens, fillerMessages)
		return
	}
	...
	fmt.Fprintf(out, "  Состав: system ≈%d · facts ≈%d · диалог ≈%d (%d сообщ.) · наполнитель ≈%d (%d блок.) · %s\n",
		systemTokens, factsTokens, dialogTokens, dialogMessages, fillerTokens, fillerMessages, space)
...
func contextCommand(agent *Agent, line string, out io.Writer) bool {
 ...
 /memory, /facts       — состояние памяти и факты
 /contextManagementStrategy — выбрать Sliding Window или Sticky Facts
 /contextManagementStrategy window|facts [N] — режим и окно (без N — 5)
 /checkpoint NAME      — сохранить точку ветвления
 /addBranch NAME [CHECKPOINT] — создать ветку от текущего места или checkpoint
 /branch [NAME]        — выбрать существующую ветку
...
Наполнитель сохраняется локально и подчиняется выбранной стратегии окна.
Введите / для меню команд. ↑/↓ — выбор; Tab — дополнить; Enter — подтвердить.
Без меню ↑/↓ перебирают историю сообщений. Esc — закрыть меню.
...
Ctrl+C — выйти; Ctrl+D — выйти на пустой строке.`)
	...
	case "/fill":
		...
		fmt.Fprintf(out, "Хранение: блок записан в историю; уйдёт в API, если к следующему запросу останется в окне. Сейчас API не вызывался. Всего наполнителя: ≈%d токенов; блоков: %d.\n", details.Tokens, details.Blocks)
		agent.ShowContext(out)
	default:
		return false
	}
